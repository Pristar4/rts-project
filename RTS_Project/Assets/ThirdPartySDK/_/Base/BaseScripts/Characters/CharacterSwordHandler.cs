		...
		EnemyHandler.IEnemyTargetable
{
	...
	private void HandleMovement()
	{
		...
		if (!isIdle)
			Dirt_Handler.SpawnInterval(GetPosition() + new Vector3(0, -4),
					moveDir * -1);
	...
	{
		var attackDir = animatedWalker.GetLastMoveVector();

		if (Input.GetMouseButtonDown(0))
			attackDir = (UtilsClass.GetMouseWorldPosition() - GetPosition())
					.normalized;
		...
		if (Input.GetKeyDown(KeyCode.Space) || Input.GetMouseButtonDown(0))
		{
			...
			*/
			...
			var activeAnimType = unitAnimation.GetActiveAnimType();
			switch (activeAnimType.GetName())
			{
				default:
					unitAnimation.PlayAnimForced(
							UnitAnimType.GetUnitAnimType(
									"dBareHands_PunchQuick"), attackDir,
							1f, unitAnim2 => SetStateNormal(), null, null);
					break;
				case "dBareHands_PunchQuick":
					unitAnimation.PlayAnimForced(
							UnitAnimType.GetUnitAnimType(
									"dBareHands_KickQuick"), attackDir, 1f,
							unitAnim2 => SetStateNormal(), null, null);
					break;
			}
