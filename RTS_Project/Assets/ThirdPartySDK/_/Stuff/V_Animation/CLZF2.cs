...
namespace V_AnimationSystem
{
	...
	/// <summary>
	///   Improved C# LZF Compressor, a very small data compression library. The
	///   compression algorithm is extremely fast.
	public static class CLZF2
	{
		...
		/// <param name="output">
		///   Reference to a buffer which will contain the compressed
		///   data
		/// </param>
		...
		/// <param name="output">
		///   Reference to a buffer which will contain the decompressed
		///   data
		/// </param>
		...
		public static int lzf_decompress(byte[] input, ref byte[] output)
		{
			...
			do
			{
				...
				if (ctrl < 1 << 5) /* literal run */
				{
					...
					if (oidx + ctrl > outputLength) //SET_ERRNO (E2BIG);
						return 0;
				...
				else /* back reference */
				{
					...
					if (oidx + len + 2 > outputLength) //SET_ERRNO (E2BIG);
						return 0;

					if (reference < 0) //SET_ERRNO (EINVAL);
						return 0;
