...
namespace V_AnimationSystem
{
	...
	public class V_Skeleton_Updater : V_ISkeleton_Updater
	{
		...
		public V_Skeleton_Updater(Mesh mesh, V_Skeleton_Anim[] loadAnims,
		                          ...
		                          V_UnitSkeleton.OnAnimTrigger OnAnyAnimTrigger,
		                          int id = -1)
		{
			this.mesh = mesh;
			this.id = id;

			var tmpAnims = new List<V_Skeleton_Anim>();
			hasVariableSortingOrder = false;
			for (var i = 0; i < loadAnims.Length; i++)
			{
				var clone = loadAnims[i].Clone();
		...
		public bool TryReplaceAnimOnBodyPart(string bodyPartName,
		                                     V_Skeleton_Anim anim)
		{
			var ret = false;
		...
		{
			var animList = new List<V_Skeleton_Anim>(anims);
		...
		public bool Update(float deltaTime)
		{
			...
			if (hasVariableSortingOrder)
				...
				for (var j = i + 1; j < animsLength; j++)
					if (anims[j].GetCurrentFrame().sortingOrder <
					    anims[i].GetCurrentFrame().sortingOrder)
					{
						var tmp = anims[j];
						anims[j] = anims[i];
						anims[i] = tmp;
		...
		{
			//Set frames to same
			for (var j = 0; j < previousAnims.Length; j++)
			{
				var anim = previousAnims[j];
				for (var i = 0; i < previousAnims.Length; i++)
				{
					var newAnim = anims[i];
					if (anim.bodyPart.Equals(newAnim.bodyPart)) //Same bodyPart
						newAnim.SetCurrentFrame(
								anim.GetCurrentFrameNumberIndex());
				}
			}
			Play();
		}
		...
		private void Play()
		{
			...
			{
				var anim = anims[i];
				var frame = anim.GetCurrentAnimFrame();

				V_UnitSkeleton.AddSquare(verticesIndex, verticesArr, uvsIndex,
						uvsArr, trianglesIndex, trianglesArr, frame.pos,
						frame.v00 + frame.v00offset,
						frame.v01 + frame.v01offset,
						frame.v10 + frame.v10offset,
						frame.v11 + frame.v11offset,
						V_UnitSkeleton.GetUV_Type(frame.uvType));
	...
	public interface V_ISkeleton_Updater
	{
		...
		bool TryReplaceAnimOnBodyPart(string bodyPartName,
		                              V_Skeleton_Anim anim);
