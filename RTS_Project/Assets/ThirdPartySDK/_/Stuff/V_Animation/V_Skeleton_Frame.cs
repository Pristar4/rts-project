...
namespace V_AnimationSystem
{
	...
	public class V_Skeleton_Frame
	{
		...
		public const string VALID_TRIGGER_DIC
		...
		public V_Skeleton_Frame(int _frameCount, Vector3 _pos, float size,
		                        float _rot, string _trigger, UVType _uvType,
		                        float scaleX, float scaleY, int sortingOrder,
		                        Vector2 pivot,
		                        Vector3 v00offset, Vector3 v01offset,
		                        Vector3 v10offset, Vector3 v11offset)
		{
		...
		public V_Skeleton_Frame(int _frameCount, Vector3 _pos, float _rot,
		                        string _trigger, UVType _uvType, float size,
		                        float scaleX, float scaleY, int sortingOrder,
		                        Vector2 pivot,
		                        Vector3 v00offset, Vector3 v01offset,
		                        Vector3 v10offset, Vector3 v11offset)
		...
		public void RefreshVertices()
		{
			...
			var heightHalf = size * uvHeightScale * scaleY;

			var eulerQuaternion = Quaternion.Euler(0, 0, rot);
			v00 = eulerQuaternion *
			      new Vector3(-size * scaleX - pivot.x * widthHalf,
					      -(size * uvHeightScale) * scaleY -
					      pivot.y * heightHalf);
			v01 = eulerQuaternion *
			      new Vector3(-size * scaleX - pivot.x * widthHalf,
					      +(size * uvHeightScale) * scaleY -
		...
		{
			return new V_Skeleton_Frame(0, pos, rot, trigger, uvType, size,
					scaleX, scaleY, sortingOrder, pivot, v00offset, v01offset,
					v10offset, v11offset);
		}
		public V_Skeleton_Frame Clone()
		{
			return new V_Skeleton_Frame(frameCount, pos, rot, trigger, uvType,
					size, scaleX, scaleY, sortingOrder, pivot, v00offset,
					v01offset, v10offset, v11offset);
		}
		public bool IsKeyframe()
		{
			return frameCount != -1;
		}
		public void SetUVTypeCoords(Vector2 v00, Vector2 v01, Vector2 v10,
		                            Vector2 v11)
		{
			uvType.uvs = new[]
			{
		...
		public static V_Skeleton_Frame[] Smooth(V_Skeleton_Frame[] keyframes)
		{
			...
			var prevFrame = keyframes[0];
			ret.Add(prevFrame);
			for (var i = 1; i < keyframes.Length; i++)
			{
				var frame = keyframes[i];
				for (var j = 1; j < frame.frameCount - 1; j++)
		...
		public static V_Skeleton_Frame GetSmoothFrame(
				V_Skeleton_Frame prevFrame, V_Skeleton_Frame frame, int index)
		{
			var posInc = (frame.pos - prevFrame.pos) / frame.frameCount;
			var rotInc = (frame.rot - prevFrame.rot) * 1f / frame.frameCount;
			var sizeInc = (frame.size - prevFrame.size) * 1f / frame.frameCount;
			var scaleXInc = (frame.scaleX - prevFrame.scaleX) * 1f /
			                frame.frameCount;
			var scaleYInc = (frame.scaleY - prevFrame.scaleY) * 1f /
			                frame.frameCount;
			var v00offsetInc = (frame.v00offset - prevFrame.v00offset) /
			                   frame.frameCount;
			var v01offsetInc = (frame.v01offset - prevFrame.v01offset) /
			                   frame.frameCount;
			var v10offsetInc = (frame.v10offset - prevFrame.v10offset) /
			                   frame.frameCount;
			var v11offsetInc = (frame.v11offset - prevFrame.v11offset) /
			                   frame.frameCount;
		...
		{
			var content = V_Animation.SplitString(save, "#SKELETONFRAME#");
			var frameCount = int.Parse(content[0]);
			var pos = V_Animation.Load_Vector3(content[1]);
			var size = float.Parse(content[2]);
			var rot = int.Parse(content[3]);
			V_Animation.StringArrPushIfIndex(4, ref content, "");
			var trigger = content[4];
			V_Animation.StringArrPushIfIndex(5, ref content, "");
			var uvType = UVType.Load(content[5]);

			V_Animation.StringArrPushIfIndex(6, ref content, "1");
			var scaleX = float.Parse(content[6]);
			V_Animation.StringArrPushIfIndex(7, ref content, "1");
			var scaleY = float.Parse(content[7]);
			V_Animation.StringArrPushIfIndex(8, ref content, "-100");
			var sortingOrder = int.Parse(content[8]);
			...
			return new V_Skeleton_Frame(frameCount, pos, size, rot, trigger,
					uvType, scaleX, scaleY, sortingOrder, pivot, v00offset,
					v01offset, v10offset, v11offset);
		}
	}
