...
namespace V_AnimationSystem
{
	...
	public class UnitAnimType
	{
		...
		private string name;
		private readonly Dictionary<UnitAnim.AnimDir, UnitAnim> singleAnimDic;
		...
		{
			if (!CanOverwrite()) // Cannot overwrite! Cannot change name!
				return;
			this.name = name;
		}
		...
		public List<string> GetTriggerList()
		{
			...
			foreach (UnitAnim.AnimDir animDir in Enum.GetValues(
					typeof(UnitAnim.AnimDir)))
			{
				var unitAnim = singleAnimDic[animDir];
		...
		public void TryFillDirections()
		{
			foreach (UnitAnim.AnimDir animDir in Enum.GetValues(
					typeof(UnitAnim.AnimDir)))
			{
				var unitAnim = UnitAnim.GetUnitAnim(name + animDir);
				if (unitAnim != null) SetAnim(animDir, unitAnim);
			}
			...
			{
				var unitAnim = UnitAnim.GetUnitAnim(name);
				if (unitAnim != null)
					foreach (UnitAnim.AnimDir animDir in Enum.GetValues(
							typeof(UnitAnim.AnimDir)))
		...
		{
			SetAnims(animDown, animDown, animDown, animDown, animDown, animDown,
					animDown, animDown);
		}
		public void SetAnims(UnitAnim animDown, UnitAnim animUp,
		                     UnitAnim animLeft, UnitAnim animRight)
		{
			SetAnims(animDown, animUp, animLeft, animRight, animDown, animDown,
					animUp, animUp);
		}
		public void SetAnims(UnitAnim animDown, UnitAnim animUp,
		                     UnitAnim animLeft, UnitAnim animRight,
		                     UnitAnim animDownLeft, UnitAnim animDownRight,
		                     UnitAnim animUpLeft, UnitAnim animUpRight)
		{
		...
		public override string ToString()
		{
			...
			foreach (UnitAnim.AnimDir animDir in Enum.GetValues(
					typeof(UnitAnim.AnimDir)))
			{
				var animName = singleAnimDic[animDir] == null
						? "null"
						: singleAnimDic[animDir].ToString();
				anims += animName + "; ";
			}
		...
		{
			var content = V_Animation.SplitString(save, "#UNITANIMTYPE#");

			var unitAnimType = new UnitAnimType(content[0]);
		...
		private static void LoadFromDataFolder()
		{
			...
			var defaultDir
					= new DirectoryInfo(V_Animation.LOC_DEFAULT_ANIMATIONTYPES);
			...
			foreach (var fileInfo in defaultFileInfoList)
			{
				...
				if (SaveSystem.Load(V_Animation.LOC_DEFAULT_ANIMATIONTYPES,
						fileInfo.Name, out fileData)) // Loaded!
					readAllText = fileData.save;
				else // Load failed!
					continue;
				var unitAnimType = Load_Linked(readAllText);
				unitAnimType.disableOverwrite = true;
				unitAnimTypeList.Add(unitAnimType);
			}

			var dir = new DirectoryInfo(V_Animation.LOC_ANIMATIONTYPES);
			...
			foreach (var fileInfo in fileInfoList)
			{
				string readAllText;
				SaveSystem.FileData fileData;
				if (SaveSystem.Load(V_Animation.LOC_ANIMATIONTYPES,
						fileInfo.Name, out fileData)) // Loaded!
					readAllText = fileData.save;
				else // Load failed!
					continue;
		...
		private static void LoadFromResources()
		{
			...
			var folderCSV = "_Default/AnimationTypes,AnimationTypes";
		...
		public static string LoadAnimTypeString(string fileNameWithExtension)
		{
			...
			SaveSystem.FileData fileData;
			if (SaveSystem.Load(V_Animation.LOC_ANIMATIONTYPES,
					fileNameWithExtension, out fileData)) // Loaded!
				readAllText = fileData.save;
			else // Load failed!
				return null;
			return readAllText;
		...
		{
			V_Animation.Init();
			foreach (var unitAnimType in unitAnimTypeList)
				if (unitAnimType.name == unitAnimTypeName)
					return unitAnimType;
			Debug.LogWarning("#### UNIT ANIM TYPE NOT FOUND: " +
			                 unitAnimTypeName);
			return null;
		}
		...
		{
			public static UnitAnimType _Test2
					= new BaseUnitAnimTypeEnum(MethodBase.GetCurrentMethod());
			...
			public static implicit operator UnitAnimType(
					BaseUnitAnimTypeEnum unitAnimTypeEnum)
			{
				return GetUnitAnimType(unitAnimTypeEnum.str);
			}
		}
