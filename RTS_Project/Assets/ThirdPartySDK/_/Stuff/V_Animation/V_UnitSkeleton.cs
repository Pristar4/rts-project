...
namespace V_AnimationSystem
{
	...
	public class V_UnitSkeleton
	{
		...
		private int id = Random.Range(10000, 99999);
		...
		private readonly float frameRateMod = 1f;

		private float refreshTimer;
		private readonly float refreshTimerMax = .016f;
		...
		public delegate void OnAnimInterrupted(UnitAnim interruptedUnitAnim,
		                                       UnitAnim unitAnim,
		                                       bool alreadyLooped);
		...
		public delegate Vector3 DelConvertLocalPositionToWorldPosition(
				Vector3 position);
		private readonly DelConvertLocalPositionToWorldPosition
		...
		public V_UnitSkeleton(float frameRateMod,
		                      DelConvertLocalPositionToWorldPosition
				                      ConvertLocalPositionToWorldPosition,
		                      Action<Mesh> SetMesh)
		{
			V_Animation.Init();
			this.frameRateMod = frameRateMod;
			this.ConvertLocalPositionToWorldPosition
					= ConvertLocalPositionToWorldPosition;
			mesh = new Mesh();
			mesh.MarkDynamic();
		...
		{
			var bodyPartAnim = GetSkeletonUpdater()
					.GetAnimWithBodyPartName(bodyPartName);
			if (bodyPartAnim != null)
			{
				var pos = bodyPartAnim.GetCurrentAnimFrame().pos;
		...
		{
			foreach (var skeletonAnim in unitAnim.GetAnims())
			{
				var replaced = TryReplaceBodyPartSkeletonAnim(skeletonAnim);
				if (!replaced) // Didn't replace, body part doesn't exist, add
					GetSkeletonUpdater().AddAnim(skeletonAnim);
			}
		}

		public void ReplaceBodyPartSkeletonAnim(UnitAnim unitAnim,
		                                        params string[] bodyPartNameArr)
		{
			foreach (var bodyPartName in bodyPartNameArr)
				TryReplaceBodyPartSkeletonAnim(bodyPartName,
						unitAnim.GetSkeletonAnim_BodyPartCustom(bodyPartName));
		}
		public bool TryReplaceBodyPartSkeletonAnim(
				string bodyPartName, UnitAnim unitAnim)
		{
			return TryReplaceBodyPartSkeletonAnim(bodyPartName,
					unitAnim.GetSkeletonAnim_BodyPartCustom(bodyPartName));
		}
		public bool TryReplaceBodyPartSkeletonAnim(
				string bodyPartName, V_Skeleton_Anim anim)
		{
			return GetSkeletonUpdater()
					.TryReplaceAnimOnBodyPart(bodyPartName, anim);
		}
		public bool TryReplaceBodyPartSkeletonAnim(V_Skeleton_Anim anim)
		{
			return GetSkeletonUpdater()
					.TryReplaceAnimOnBodyPart(anim.bodyPart.customName, anim);
		}
		...
		{
			var activeSkeletonUpdater = skeletonUpdater;
			var allLooped = skeletonUpdater.Update(deltaTime);

			if (activeSkeletonUpdater !=
			    skeletonUpdater) // Skeleton Updater changed during update
				return;
			...
			if (allLooped) alreadyLooped = true;
			if (allLooped && onAnimComplete != null)
			{
				var backOnAnimComplete = onAnimComplete;
				onAnimComplete = null;
		...
		public Vector3 GetAnimPointer(int pointer)
		{
			switch (pointer)
			{
				default:
				case 1:
					return ConvertLocalPositionToWorldPosition(animPointer_1);
				case 2:
					return ConvertLocalPositionToWorldPosition(animPointer_2);
				case 3:
					return ConvertLocalPositionToWorldPosition(animPointer_3);
			}
		}
		...
		public bool ClearOnAnimInterruptedIfMatches(
				OnAnimInterrupted onAnimInterrupted)
		{
		...
		public bool PlayAnimIfOnCompleteMatches(UnitAnim unitAnim,
		...
		public void PlayAnim(UnitAnim unitAnim, float frameRateModModifier,
		                     OnAnimComplete onAnimComplete,
		                     OnAnimTrigger onAnimTrigger,
		                     OnAnimInterrupted onAnimInterrupted)
		{
			if (this.onAnimInterrupted != null)
			{
				var tmpAnimInterrupted = this.onAnimInterrupted;
				this.onAnimInterrupted = null;
			...
			// Dynamic anim
			var newSkeletonUpdater = new V_Skeleton_Updater(mesh,
					unitAnim.GetAnims(), frameRateMod, frameRateModModifier,
		...
		public void PlayAnimContinueFrames(UnitAnim unitAnim,
		                                   float frameRateModModifier,
		                                   OnAnimComplete onAnimComplete,
		                                   OnAnimTrigger onAnimTrigger,
		                                   OnAnimInterrupted onAnimInterrupted)
		{
			...
			if (skeletonUpdater is V_Skeleton_Updater)
			{
				...
				var newSkeletonUpdater = new V_Skeleton_Updater(mesh,
						unitAnim.GetAnims(), frameRateMod, frameRateModModifier,
						onAnimTrigger, OnAnyAnimTrigger);
				newSkeletonUpdater.SetFramesToSame(
						(skeletonUpdater as V_Skeleton_Updater).GetAnims());
				skeletonUpdater = newSkeletonUpdater;
			}
			else
			{
				// Previous was cached updater, ignore continue frames
				skeletonUpdater = new V_Skeleton_Updater(mesh,
						unitAnim.GetAnims(), frameRateMod, frameRateModModifier,
						onAnimTrigger, OnAnyAnimTrigger);
			}
		}
		...
		public static void AddSquare(int verticesIndex, Vector3[] verticesArr,
		                             int uvsIndex, Vector2[] uvsArr,
		                             int trianglesIndex, int[] trianglesArr,
		                             Vector3 localPos, Vector3 v00, Vector3 v01,
		                             Vector3 v10, Vector3 v11,
		                             Vector2[] squareUV)
		{
			/* 01
			 * 11
			...
			for (var i = 0; i < squareUV.Length; i++)
				uvsArr[uvsIndex + i] = squareUV[i];
		}
		public static void AddSquare(List<Vector3> verticesList,
		                             List<Vector2> uvsList,
		                             List<int> trianglesList, Vector3 localPos,
		                             Vector3 v00, Vector3 v01, Vector3 v10,
		                             Vector3 v11, Vector2[] squareUV)
		{
			 ...
			 * 00
			 * 10 */
			var verticesIndex = verticesList.Count;
			...
			for (var i = 0; i < squareUV.Length; i++) uvsList.Add(squareUV[i]);
		}
