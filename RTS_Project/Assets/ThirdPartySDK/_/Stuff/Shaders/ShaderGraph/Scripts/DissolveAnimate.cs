...
public class DissolveAnimate : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (dissolveAction != null) dissolveAction.Update(dissolveAmount);
	}
	...
	public void StartDissolve(float startDissolveAmount, float dissolveSpeed,
	                          DissolveAction dissolveAction = null)
	{
	...
	private void RefreshMaterial()
	{
		if (transform.Find("Body") != null)
			material = transform.Find("Body").GetComponent<MeshRenderer>()
					.material;
	...
	public class DissolveAction
	{
		...
		private readonly float targetDissolveAmount;
		private readonly bool triggerOnce;
		private bool alreadyTriggeredOnce;

		public DissolveAction(float targetDissolveAmount, Condition condition,
		                      Action action, bool triggerOnce)
		{
		...
		{
			if (triggerOnce &&
			    alreadyTriggeredOnce) // Should only trigger once and it already triggered once
				return;
			...
			switch (condition)
			{
				...
				case Condition.Greater:
					isConditionTrue = dissolveAmount >= targetDissolveAmount;
					break;
				case Condition.Less:
					isConditionTrue = dissolveAmount <= targetDissolveAmount;
					break;
