...
public class Window_Graph_Object : MonoBehaviour
{
	...
	{
		if (getValueFunc == null) getValueFunc = GetValueFuncDefault;
	...
	public void ShowGraph(IList valueList, IGraphVisual graphVisual = null,
	                      int maxVisibleValueAmount = -1,
	                      Func<int, string> getAxisLabelX = null,
	                      Func<float, string> getAxisLabelY = null)
	{
		Setup();
		if (valueList == null) valueList = new List<object> { 0 };
				...
				i++)
		{
			...
			var yPosition = (GetValue(valueList[i]) - yMinimum) /
					(yMaximum - yMinimum) * graphHeight;
		...
		if (!yScaleChanged)
		{
			...
			var xIndex
					= index - Mathf.Max(valueList.Count - maxVisibleValueAmount,
							0);
			var xPosition = xSize + xIndex * xSize;
			var yPosition = (GetValue(value) - yMinimum) /
			                (yMaximum - yMinimum) *
			                graphHeight;

			// Add data point visual
			var tooltipText = getAxisLabelY(GetValue(value));
	...
	public void UpdateAllVisiblePoints()
	{
				...
				i++)
		{
			...
			// Add data point visual
			var tooltipText = getAxisLabelY(GetValue(valueList[i]));
			graphVisualObjectList[xIndex]
					.SetGraphVisualObjectInfo(new Vector2(xPosition, yPosition),
							xSize,
							tooltipText);
	...
	private void CalculateYScale(out float yMinimum, out float yMaximum)
	{
		...
		if (fixedYMinimum != null) yMinimum = fixedYMinimum.Value;
		if (fixedYMaximum != null) yMaximum = fixedYMaximum.Value;
	}
