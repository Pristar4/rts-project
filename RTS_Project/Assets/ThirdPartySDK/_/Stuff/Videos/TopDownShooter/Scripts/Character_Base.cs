	...
	public class Character_Base : MonoBehaviour, ICharacterAnims
	{
		...
		public void PlayJumpAnim(Vector3 moveDir)
		{
			if (moveDir.x >= 0)
				...
				unitAnimation.PlayAnim(
						UnitAnim.GetUnitAnim("dBareHands_JumpLeft"));
		}
		                               ...
		                               float frameRateMod = 1f)
		{
			unitAnimation.PlayAnimForced(
					UnitAnimType.GetUnitAnimType("dBareHands_PunchQuick"), dir,
					1f,
					unitAnim2 =>
					{
					...
					}, trigger =>
					{
						...
						// HIT2 = HandL
						var hitBodyPartName
								= trigger == "HIT" ? "HandR" : "HandL";
						var impactPosition
								= unitSkeleton.GetBodyPartPosition(
										hitBodyPartName);
		...
		{
			unitAnimation.PlayAnimForced(
					UnitAnimType.GetUnitAnimType("dBareHands_KickQuick"), dir,
					1f,
					unitAnim2 =>
					{
						if (onAnimComplete != null) onAnimComplete();
		...
		{
			unitAnimation.PlayAnimForced(
					UnitAnimType.GetUnitAnimType("Spiderman_ShootWebZip"), dir,
					1f, null,
					null, null);
		}
		...
		{
			unitAnimation.PlayAnimForced(
					UnitAnimType.GetUnitAnimType("Spiderman_WebZipFlying"), dir,
					1f, null,
					null, null);
		}
		...
		{
			unitAnimation.PlayAnimForced(
					UnitAnimType.GetUnitAnimType("dBareHands_Roll"), dir, 1f,
					null, null,
					null);
		}
		...
		{
			unitAnimation.PlayAnimForced(
					UnitAnim.GetUnitAnim("dBareHands_Victory"),
					1f, null);
		}
		...
		private void Awake()
		{
			...
			unitSkeleton = new V_UnitSkeleton(1f, bodyTransform.TransformPoint,
					mesh => bodyTransform.GetComponent<MeshFilter>().mesh
							= mesh);
			...
			var attackUnitAnim
					= UnitAnimType.GetUnitAnimType(
							"dBareHands_PunchQuickAttack");

			animatedWalker = new AnimatedWalker(unitAnimation, idleUnitAnim,
					walkUnitAnim, 1f, 1f);
		}
		...
		{
			var bodyTransform = transform.Find("Body");
			bodyTransform.GetComponent<MeshFilter>().mesh
					= unitSkeleton.GetMesh();
		}
