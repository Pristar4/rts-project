...
namespace TopDownShooter
{
	public class EnemyTurretLogic : MonoBehaviour
	{
		...
		private void Update()
		{
			switch (state)
			{
				case State.Normal:
					...
					if (enemyTarget != null)
					{
						...
						aimingShoot.StartAimingAtTarget(
								enemyTarget.GetPosition(), 20f, 0f,
								10f, ShootTarget);
					}
					break;
				...
				case State.Attacking:
					...
					if (fireRate <= 0f) TestContinueShooting();
					break;
				case State.Busy:
					break;
			}
		...
		private void Player_OnDodged(object sender, EventArgs e)
		{
			if (state == State.AimingAtTarget &&
			    aimingShoot.IsInsideDodgeAngle())
				SetStateNormal();
		}
		                                 ...
		                                 EnemyMain.OnDamagedEventArgs e)
		{
			...
			enemyMain.HealthSystem.Damage(
					Mathf.RoundToInt(30 * e.damageMultiplier));

			if (
					enemyMain.HealthSystem
							.IsDead()) //FlyingBody.Create(GameAssets.i.pfEnemyFlyingBody, GetPosition(), bloodDir);
		...
		private void ShootTarget()
		{
			...
			{
				// Shoot!
				Sound_Manager.PlaySound(Sound_Manager.Sound.Rifle_Fire,
						GetPosition());
				var targetPosition = enemyTarget.GetPosition();
				...
				fireRate = FIRE_RATE;
				targetPosition
						+= UtilsClass.GetRandomDir() * Random.Range(-5f, 15f);
