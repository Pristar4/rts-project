...
public class Generic_Mesh_Script : MonoBehaviour
{
	...
	private static Dictionary<string, Generic_Mesh_Script> dictionary
			= new Dictionary<string, Generic_Mesh_Script>();

	private static Quaternion[] cachedQuaternionEulerArr;
	public Bounds bounds;
	public int amtMax = 15000;
	public Vector2 UVTextureSize = new Vector2(1, 1);
	public bool spawnNewMeshWhenFull;
	...
	private void Awake()
	{
		...
		{
			presetUV[i * 4 + 0] = new Vector2(
					UVList[i].Vector_1.x / UVTextureSize.x,
					UVList[i].Vector_1.y / UVTextureSize.y);
			presetUV[i * 4 + 1] = new Vector2(
					UVList[i].Vector_2.x / UVTextureSize.x,
					UVList[i].Vector_2.y / UVTextureSize.y);
			presetUV[i * 4 + 2] = new Vector2(
					UVList[i].Vector_3.x / UVTextureSize.x,
					UVList[i].Vector_3.y / UVTextureSize.y);
			presetUV[i * 4 + 3] = new Vector2(
					UVList[i].Vector_4.x / UVTextureSize.x,
	...
	private void addGeneric(Vector3 pos, float rot, int material, float offset,
	                        Vector3 baseDir, bool skewed)
	{
		...
		if (skewed)
		{
			vertices[vIndex0] = pos +
			                    GetQuaternionEuler(rot) *
			                    new Vector3(-baseDir.x, baseDir.y);
			vertices[vIndex1] = pos + GetQuaternionEuler(rot) *
					new Vector3(-baseDir.x, -baseDir.y);
			vertices[vIndex2] = pos +
			                    GetQuaternionEuler(rot) *
			                    new Vector3(baseDir.x, -baseDir.y);
		...
		if (index == 0 && spawnNewMeshWhenFull)
		{
			...
			var newInstance = new GameObject(name, typeof(MeshFilter),
					typeof(MeshRenderer), typeof(Generic_Mesh_Script));
			var trans = newInstance.transform;
			trans.parent = transform.parent;
			trans.localPosition = transform.localPosition;

			newInstance.GetComponent<Renderer>().material
					= transform.GetComponent<MeshRenderer>().material;
			newInstance.GetComponent<Renderer>().sortingOrder
					= transform.GetComponent<Renderer>().sortingOrder;

			var meshScript = newInstance.GetComponent<Generic_Mesh_Script>();
			...
			for (var i = 0; i < 999; i++)
			{
				if (transform.parent.Find(name + "_" + number) == null) break;
				number++;
			}
			trans.name = name;
			name = name + "_" + number;
	...
	public void updateGeneric(int index, Vector3 pos, float rot, int material,
	                          float offset, Vector3 baseDir, bool skewed)
	{
		update = true;
		//pos.z -= offset;
	...
	public static void AddGeneric(string meshName, Vector3 pos, float rot,
	                              int material, float offset, Vector3 baseDir,
	                              bool skewed)
	{
		dictionary[meshName]
				.addGeneric(pos, rot, material, offset, baseDir, skewed);
	}
	public static void UpdateGeneric(string meshName, int index, Vector3 pos,
	                                 float rot, int material, float offset,
	                                 Vector3 baseDir, bool skewed)
	{
		dictionary[meshName]
				.updateGeneric(index, pos, rot, material, offset, baseDir,
						skewed);
	}
