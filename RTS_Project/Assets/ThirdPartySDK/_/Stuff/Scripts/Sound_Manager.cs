...
public static class Sound_Manager
{
	...
	private static bool disableAllSounds;

	private static GameObject globalGameObject;
	private static DelGetAudio GetAudioFunc;
	private static GameObject[] poolArr;
	private static int poolAmount = 30;
	private static readonly int poolAmountMax = 50;
	...
	public static void Load(Sound sound, string folder = "")
	{
		...
		if (dictionary[sound] == null)
			Debug.Log("Couldnt load " + folder + sound);
		timers[sound] = 0f;
	}
	...
	public static AudioSource PlaySound(Sound sound, Vector3 pos = default,
	                                    object obj = null)
	{
		...
		switch (sound)
		{
			...
			case Sound.Dash:
				Sound[] sounds =
				{
					Sound.Dash, Sound.Dash_2, Sound.Dash_3, Sound.Dash_4
				};
				return PlayClip(
						dictionary[sounds[Random.Range(0, sounds.Length)]]);
			...
			case Sound.Sword:
				sounds = new[]
				{
					Sound.Sword, Sound.Sword_2, Sound.Sword_3, Sound.Sword_4,
					Sound.Sword_5,
					Sound.Sword_6, Sound.Sword_7
				};
				return PlayClip(
						dictionary[sounds[Random.Range(0, sounds.Length)]]);
			//return PlayClipAtPoint(dictionary[sounds[Random.Range(0, sounds.Length)]], pos, 100);
			...
			case Sound.Footstep_Minion:
				sounds = new[]
				{
					Sound.Footstep_Minion, Sound.Footstep_Minion_2,
					Sound.Footstep_Minion_3,
					Sound.Footstep_Minion_4, Sound.Footstep_Minion_5,
					Sound.Footstep_Minion_6
				};
				return PlayClipAtPoint(
						dictionary[sounds[Random.Range(0, sounds.Length)]],
						pos, 140, .5f * GetAudio(AudioType.Sound));
			...
			case Sound.Player_Hit:
				sounds = new[]
				{
					Sound.Player_Hit, Sound.Player_Hit_2, Sound.Player_Hit_3,
					Sound.Player_Hit_4, Sound.Player_Hit_5
				};
				return PlayClipAtPoint(
						dictionary[sounds[Random.Range(0, sounds.Length)]],
						pos, 100);
			...
			case Sound.Minion_Sword:
				...
				timers[sound] = Time.realtimeSinceStartup +
				                Random.Range(.05f, .2f);
				sounds = new[]
				{
					Sound.Minion_Sword, Sound.Minion_Sword_2,
					Sound.Minion_Sword_3,
					Sound.Minion_Sword_4, Sound.Minion_Sword_5,
					Sound.Minion_Sword_6,
					Sound.Minion_Sword_7, Sound.Minion_Sword_8,
					Sound.Minion_Sword_9,
					Sound.Minion_Sword_10, Sound.Minion_Sword_11
				};
				return PlayClipAtPoint(
						dictionary[sounds[Random.Range(0, sounds.Length)]],
						pos);
			...
			case Sound.Minion_Dead:
				...
				sounds = new[]
				{
					Sound.Minion_Dead, Sound.Minion_Dead_2, Sound.Minion_Dead_3
				};
				return PlayClipAtPoint(
						dictionary[sounds[Random.Range(0, sounds.Length)]],
						pos);
			...
			case Sound.Axe_1:
				...
				sounds = new[]
				{
					Sound.Axe_1, Sound.Axe_2, Sound.Axe_3, Sound.Axe_4,
					Sound.Axe_5,
					Sound.Axe_6
				};
				return PlayClipAtPoint(
			...
			case Sound.Pickaxe_1:
				...
				sounds = new[]
				{
					Sound.Pickaxe_1, Sound.Pickaxe_2, Sound.Pickaxe_3,
					Sound.Pickaxe_4,
					Sound.Pickaxe_5
				};
				return PlayClipAtPoint(
			...
			case Sound.Rifle_Fire:
				...
				*/
				...
				sounds = new[]
				{
					Sound.Rifle_Fire_1, Sound.Rifle_Fire_2, Sound.Rifle_Fire_3,
					Sound.Rifle_Fire_4
				};
				return PlayClipAtPoint(
						dictionary[sounds[Random.Range(0, sounds.Length)]],
	...
	{
		var src = PlayClipAtPoint(clip, Vector3.zero);
	...
	{
		var src = PlayClipAtPoint(clip, Vector3.zero, 128,
				GetAudio(AudioType.Sound), loop);
		src.spatialBlend = 0f;
		return src;
	}
	private static AudioSource PlayClip(AudioClip clip, int priority)
	{
		var src = PlayClipAtPoint(clip, Vector3.zero, priority,
				GetAudio(AudioType.Sound));
		src.spatialBlend = 0f;
		return src;
	}
	...
	private static AudioSource PlayClipAtPoint(AudioClip clip, Vector3 pos,
	                                           int priority)
	{
		return PlayClipAtPoint(clip, pos, priority, GetAudio(AudioType.Sound),
				false);
	}
	private static AudioSource PlayClipAtPoint(AudioClip clip, Vector3 pos,
	                                           int priority,
	                                           float volMultiplier)
	{
		return PlayClipAtPoint(clip, pos, priority,
				GetAudio(AudioType.Sound) * volMultiplier, false);
	}
	private static AudioSource PlayClipAtPoint(AudioClip clip, Vector3 pos,
	                                           int priority, float vol,
	                                           bool loop = false)
	{
		var tempGO = GetPooledObject();
	...
	public static void PoolObjects()
	{
		...
		{
			var tempGO
					= new GameObject("Pooled_Audio_" + i,
							typeof(AudioSource)); // create the temp object
	...
	{
		if (poolArr == null || poolArr.Length < poolAmount ||
		    poolArr[0] == null)
			PoolObjects();
		for (var i = 0; i < poolAmount; i++)
			if (poolArr[i] != null && !poolArr[i].activeSelf)
				return poolArr[i];
		if (poolAmount >=
		    poolAmountMax) // Maximum objects in pool, dont create more!
			return poolArr[Random.Range(0, poolArr.Length)];
		var tempGO
				= new GameObject("Pooled_Audio_" + poolAmount,
						typeof(AudioSource)); // create the temp object
		//tempGO.AddComponent<AudioSource>(); // add an audio source
		tempGO.SetActive(false);
		var poolList = new List<GameObject>(poolArr);
		poolList.Add(tempGO);
		poolAmount++;
		poolArr = poolList.ToArray();
		return tempGO;
