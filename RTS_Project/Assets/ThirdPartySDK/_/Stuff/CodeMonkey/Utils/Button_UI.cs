...
namespace CodeMonkey.Utils
{
			...
			IPointerUpHandler
	{
		...
		public Action
				MouseOverPerSecFunc = null; //Triggers every sec if mouseOver
		...
		private void Awake()
		{
			...
			posEnter = (Vector2)transform.localPosition +
 ...
 () => { if (cursorMouseOut != CursorManager.CursorType.None) CursorManager.SetCursor(cursorMouseOut); };
		...
		public void SetHoverBehaviourType(HoverBehaviour hoverBehaviourType)
		{
			...
			switch (hoverBehaviourType)
			{
				...
				case HoverBehaviour.Change_Image:
					hoverBehaviourFunc_Enter = delegate
					{
						hoverBehaviour_Image.sprite
								= hoverBehaviour_Sprite_Enter;
					};
					hoverBehaviourFunc_Exit = delegate
					{
						hoverBehaviour_Image.sprite
								= hoverBehaviour_Sprite_Exit;
					};
					break;
		...
		public InterceptActionHandler InterceptActionClick(
		...
		public InterceptActionHandler InterceptAction(
				string fieldName, Func<bool> testPassthroughFunc)
		{
			return InterceptAction(GetType().GetField(fieldName),
					testPassthroughFunc);
		}
		public InterceptActionHandler InterceptAction(
				FieldInfo fieldInfo, Func<bool> testPassthroughFunc)
		{
			var backFunc = fieldInfo.GetValue(this) as Action;
			var interceptActionHandler
					= new InterceptActionHandler(()
