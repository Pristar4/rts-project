...
namespace CodeMonkey.Utils
{
	...
	public class FunctionPeriodic
	{
		...
		private static List<FunctionPeriodic>
				funcList; // Holds a reference to all active timers
		...
		private FunctionPeriodic(GameObject gameObject, Action action,
		                         float timer,
		                         Func<bool> testDestroy, string functionName,
		...
		public static FunctionPeriodic Create_Global(
				Action action, Func<bool> testDestroy, float timer)
		{
			var functionPeriodic
					= Create(action, testDestroy, timer, "", false, false,
							false);
		...
		public static FunctionPeriodic Create(Action action,
		...
		public static FunctionPeriodic Create(Action action, float timer,
		                                      string functionName)
		{
			return Create(action, null, timer, functionName, false, false,
					false);
		}

		public static FunctionPeriodic Create(Action callback,
		                                      Func<bool> testDestroy,
		                                      float timer,
		                                      string functionName,
		                                      bool stopAllWithSameName)
		{
			return Create(callback, testDestroy, timer, functionName, false,
					false,
					stopAllWithSameName);
		}
		...
		public static FunctionPeriodic Create(Action action,
		                                      Func<bool> testDestroy,
		                                      float timer, string functionName,
		                                      bool useUnscaledDeltaTime,
		                                      ...
		                                      bool stopAllWithSameName)
		{
			...
			var functionPeriodic = new FunctionPeriodic(gameObject, action,
					timer,
					testDestroy, functionName, useUnscaledDeltaTime);
			gameObject.GetComponent<MonoBehaviourHook>().OnUpdate
					= functionPeriodic.Update;
