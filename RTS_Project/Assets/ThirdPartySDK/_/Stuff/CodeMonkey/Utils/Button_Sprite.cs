...
namespace CodeMonkey.Utils
{
	...
	public class Button_Sprite : MonoBehaviour
	{
		...
		public void SetHoverBehaviourChangeColor(
				Color colorOver, Color colorOut)
		{
		...
		private void Update()
		{
			if (draggingMouseRight)
				if (MouseRightDragUpdateFunc != null)
					MouseRightDragUpdateFunc(mouseRightDragStart,
							GetWorldPositionFromUI());
			if (Input.GetMouseButtonUp(1))
			{
				if (draggingMouseRight)
				{
					...
					if (MouseRightDragFunc != null)
						MouseRightDragFunc(mouseRightDragStart,
								GetWorldPositionFromUI());
				}
		...
		private void Awake()
		{
			if (GetWorldCamera == null)
				SetGetWorldCamera(()
						=> Camera.main); // Set default World Camera
			posExit = transform.localPosition;
			posEnter = transform.localPosition +
			           (Vector3)hoverBehaviour_Move_Amount;
			SetupHoverBehaviour();
 ...
 () => { if (cursorMouseOver != CursorManager.CursorType.None) CursorManager.SetCursor(cursorMouseOver); };
		...
		{
			var worldPosition
					= GetWorldCamera().ScreenToWorldPoint(Input.mousePosition);
