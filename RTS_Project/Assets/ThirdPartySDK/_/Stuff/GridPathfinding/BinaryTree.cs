	...
	public class BinaryTree
	{
		...
		public void AddNode(PathNode pathNode)
		{
			...
			else
			{
				// Has root
				//debugString = "";
				try { addNode(root, pathNode, pathNode.fValue); }
				catch (Exception e)
				{
					Debug.Log(e);
					...
					else
					{
						...
						else
						{
							...
							else
							{
								...
								if (leafRight.right.left !=
								    null) // This leaf has a left
		...
		private void removeNode(BinaryNode node, PathNode pathNode,
		                        int pathFvalue)
		{
			if (pathFvalue <= node.pathNode.fValue)
			{
		...
		public PathNode GetSmallest()
		{
			...
			var count = 0;
			try { pathNode = getSmallest(root, ref count); }
			catch
			{
		...
		private class BinaryNode
		{
			...
			public BinaryNode(PathNode _pathNode, BinaryNode _left,
			                  BinaryNode _right)
			{
				//id = nextId++;
				pathNode = _pathNode;
