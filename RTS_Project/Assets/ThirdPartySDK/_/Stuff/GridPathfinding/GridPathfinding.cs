...
namespace GridPathfindingSystem
{
	...
	public class GridPathfinding
	{
		...
		public delegate void OnPathCallback(List<PathNode> path,
		                                    MapPos finalPos);
		...
		public GridPathfinding(Vector3 worldLowerLeft, Vector3 worldUpperRight,
		                       float nodeSize)
		{
			instance = this;
		...
		public GridPathfinding(int mapWidth, int mapHeight, float nodeSize,
		                       Vector3 worldOrigin)
		{ //, Texture2D map) {
			this.nodeSize = nodeSize;
			...
			for (int i = 0; i < widthMax; i++)
			{
				for (int j = 0; j < heightMax; j++)
				{
					Vector3 nodeWorldPosition
							= mapNodes[i][j]
									.GetWorldVector(worldOrigin, nodeSize);
					RaycastHit2D raycastHit
							= Physics2D.Raycast(nodeWorldPosition, Vector2.zero,
									0f);
		...
		public void PrintMap(Transform prefabWalkable,
		                     Transform prefabUnwalkable)
		{
			for (int x = 0; x < mapNodes.Length; x++)
			{
				for (int y = 0; y < mapNodes[x].Length; y++)
				{
					...
					Object.Instantiate(
							pathNode.IsWalkable()
									? prefabWalkable
									: prefabUnwalkable,
							new Vector3(x * nodeSize, y * nodeSize),
							Quaternion.identity);
				}
			}
		}
		...
		public void PrintMap(Action<int, int> printWalkable,
		                     Action<int, int> printUnwalkable)
		{
			for (int x = 0; x < mapNodes.Length; x++)
			{
				for (int y = 0; y < mapNodes[x].Length; y++)
				{
					PathNode pathNode = mapNodes[x][y];
					if (pathNode.IsWalkable()) { printWalkable(x, y); }
					else
					{
						printUnwalkable(x, y);
					}
		...
		public void PrintMap(Action<Vector3, Vector3, Color> createSprite)
		{
			PrintMap(
					...
					{
						createSprite(
								worldOrigin + new Vector3(x * nodeSize,
										y * nodeSize),
								new Vector3(2, 2), Color.green);
		...
		private bool IsValidShortcut(int startX, int startY, int endX, int endY)
		{
			...
			Vector3 dir
					= (new Vector3(endX, endY) - new Vector3(startX, startY))
					.normalized;
			...
			while (!(testX == endX && testY == endY))
			{
				if (!IsWalkable(testX, testY) ||
				    mapNodes[testX][testY].weight != shortcutWeight)
				{
					// Not walkable
		...
		public List<PathNode> GetFindPath(MapPos startPos, MapPos finalPos)
		{
			...
			if (startPos.x < 0 || startPos.y < 0 || finalPos.x < 0 ||
			    finalPos.y < 0 ||
		...
		public List<PathNode> GetFindPathClosest(MapPos startPos,
		                                         List<MapPos> allFinalPos)
		{
		...
		public bool FindPath(int startX, int startY, MapPos finalPos,
		                     OnPathCallback callback)
		{
			return FindPath(startX, startY, new List<MapPos>() { finalPos },
					callback);
		}

		public bool FindPath(int startX, int startY,
		                     List<MapPos> finalPositions,
		                     OnPathCallback callback)
		{
			int width = widthMax;
			int height = heightMax;
			...
			{
				if (startX < 0 || startY < 0 || finalPos.x < 0 ||
				    finalPos.y < 0 ||
				...
				List<PathNode> currentPath
						= findPath(startX, startY, finalPos.x, finalPos.y);
				if (currentPath.Count <= 0 &&
				    (startX != finalPos.x || startY != finalPos.y))
				{
				...
				else
				{
					...
					paths.Add(new PathRoute(currentPath, worldOrigin, nodeSize,
							finalPos));
				}
			}
			int smallest = 0;
			...
			{
				if (paths[i].pathNodeList.Count <
				    paths[smallest].pathNodeList.Count)
					smallest = i;
			}

			if (paths.Count <= 0 ||
			    (paths.Count > 0 && paths[smallest].pathNodeList.Count <= 0))
			{
				// No path
				return false;
			}
			...
			{
				callback(paths[smallest].pathNodeList,
						paths[smallest].finalPos);
			}
			return true;
		}
		...
		public Vector3 GetClosestValidPosition(Vector3 position)
		{
			...
			PathNode pathNode
					= mapNodes[closestValidMapPos.x][closestValidMapPos.y];
		...
		public void ApplyShortcuts(ref List<PathNode> pathNodeList)
		{
			if (pathNodeList.Count > 1)
			{
				...
				while (testStartNodeIndex < pathNodeList.Count - 2)
				{
					...
					while (IsValidShortcut(testStartNode.xPos,
							testStartNode.yPos,
							testEndNode.xPos, testEndNode.yPos))
					{
						// Valid shortcut
		...
		public List<PathNode> GetPath(Vector3 start, Vector3 end)
		{
			...
			MapPos startMapPos = GetClosestValidPos(Mathf.RoundToInt(start.x),
					Mathf.RoundToInt(start.y));
			MapPos endMapPos = GetClosestValidPos(Mathf.RoundToInt(end.x),
					Mathf.RoundToInt(end.y));
			return findPath(startMapPos.x, startMapPos.y, endMapPos.x,
					endMapPos.y);
		}
		...
		private void DetermineNodeValues(PathNode currentNode, PathNode testing,
		                                 PathNode targetNode)
		{
			...
			// Ignore Walls
			if (currentNode.weight == WALL_WEIGHT ||
			...
			if (!testing.isOnClosedList)
			{
				...
				if (testing.isOnOpenList)
				{
					...
					int newGcost = currentNode.gValue + currentNode.weight +
					               movementCost;
				...
				else
				{
					...
					testing.parent = currentNode;
					testing.gValue
							= currentNode.gValue + currentNode.weight +
							  movementCost;
					testing.CalculateFValue();
		...
		private void CalculateManhattanDistance(PathNode currentNode, int currX,
		                                        int currY, int targetX,
		                                        int targetY)
		{
			currentNode.parent = null;
			currentNode.hValue
					= (Mathf.Abs(currX - targetX) + Mathf.Abs(currY - targetY));
			currentNode.isOnOpenList = false;
		...
		private void CalculateAllHeuristics(int endX, int endY)
		{
			...
			for (int x = 0; x < cols; x++)
			{
				...
				{
					CalculateManhattanDistance(mapNodes[x][y], x, y, endX,
							endY);
				}
			}
		}
		...
		private void ConvertVectorPosition(Vector3 position, out int x,
		                                   out int y)
		{
			x = (int)((position.x - worldOrigin.x) / nodeSize);
			y = (int)((position.y - worldOrigin.y) / nodeSize);
		}
		private void ConvertVectorPositionValidate(Vector3 position, out int x,
		                                           out int y)
		{
