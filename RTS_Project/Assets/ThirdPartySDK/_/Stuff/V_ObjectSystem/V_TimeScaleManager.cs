...
namespace V_ObjectSystem
{
	...
	public static class V_TimeScaleManager
	{
		...
		public static void Init()
		{
			...
			foreach (TimeType en in Enum.GetValues(typeof(TimeType)))
		...
		private static void UpdateTimeScale()
		{
			...
			{
				// Sort by priority
				for (var i = 0; i < activeEffectsList.Count; i++)
				for (var j = i + 1; j < activeEffectsList.Count; j++)
					if (GetEffectPriority(activeEffectsList[j]) >
					    GetEffectPriority(activeEffectsList[i]))
					{
						var tmp = activeEffectsList[i];
			...
			foreach (TimeType en in Enum.GetValues(typeof(TimeType)))
				deltaTimeModifierDic[en] = 1f;
			if (HasEffect(Effect.Hero_Intro) ||
			    HasEffect(Effect.Window_GameMenu) ||
			    HasEffect(Effect.Window_Map))
			{
				deltaTimeModifierDic[TimeType.Combo_SuperSelfTimer] = 0f;
				deltaTimeModifierDic[TimeType.Combo_RefreshTimer] = 0f;
			}
			if (HasEffect(Effect.Window_GameMenu) ||
			    HasEffect(Effect.Window_Map))
